...
module.exports = {
    ...
    async execute(interaction) {
        database.connect(async (err, dbClient) => {
            ...
            var args = interaction.options.getString('input')
                    ...
                    if (emails.includes(args)) {
                        collection.find({ _id: interaction.user.id }).toArray(async function (err, res) {
                            if (res.length === 0) { // no records for this person - send email & generate code
                                ...
                                interaction.reply(`Verification code has been sent to the email provided. Go check your inbox. Make sure to check your spam folders if you don't receive the message!`)
                        ...
                        interaction.reply(`Please execute the \`-verify\` command followed by a space then followed by your CPS 23 email. For questions, send a message in #helpline.`)
                    }
                } else {
                    interaction.reply(`Please execute the \`-verify\` command followed by a space then followed by your CPS email. For questions, send a message in #helpline.`)
                }
            } else if (args.trim().length === 6 && /^\d+$/.test(args.trim())) {
                collection.find({ _id: interaction.user.id }).toArray(async function (err, res) {
                    if (args.trim() == res[0].code) { // verification successful
                        interaction.reply(`Verification Successful!`)
                        ...
                        interaction.reply(`Code is wrong, please try again. Message in #helpline if you have questions.`)
                    }
                })
            }
        })
